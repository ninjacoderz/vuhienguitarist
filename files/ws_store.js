var WSCartDialog = (function(settings) {
	...
	var _wsDialog = {
		show: function() {
			...
			if(typeof parent != 'undefined' && parent.App) {
				parent.App.vent.once('preview', this.hide);
			}

			// hide free banners
			$('a[id*="ws-bottom-banner"]').hide();

			// hide facebook chat dialog
		...
		render: function() {
			...
			if(items.length) {
				...
				for(var j = 0; j < items.length; j++) {
					...
					if(itemDetail) {
						...
						if(choices) {
							for(var i = 0; i < choices.length; i++) {
								...
								for(var choiceKey in choice) {
									choicesLabel += '<b>' + encodeForHTML(choiceKey) + ": </b>" + encodeForHTML(choice[choiceKey]) + "<br/>";
								}
							}
						}
						...
						if(choicesLabel || (item.get('customTextLabel') && item.get('customText')) || fields.length) {
							html +=			'<div>';

							if(choicesLabel) {
								html += '<div>' + choicesLabel + '</div>';
							}

							if(item.get('customTextLabel') && item.get('customText')) {
								html += $('<div><div><b>' + encodeForHTML(item.get('customTextLabel')) + ': </b>' + encodeForHTML(item.get('customText')) + '</div></div>').html();
							}

							if(fields.length) {
								for(var i = 0; i < fields.length; i++) {
									var field = fields[i];
									// let fieldLabel = '<b>' + encodeForHTML(field.label) + ':</b>' + encodeForHTML(field.value);

									// html += '<div>' + fieldLabel + '</div>';

									html += $('<div><div><b>' + encodeForHTML(field.label) + ': </b>' + encodeForHTML(field.value) + '</div></div>').html();
								}
							}
						...
						*/
						...
						html += 	$('<span><input name="sm-cart-item-fields-' + numOfValidItems + '" type="hidden" class="sm-cart-item-fields" value=""></span>').find('input').val(JSON.stringify(fields)).parent().html();
			...
			// members checkout
			// check if enabled, if yes, then show dialog
			if(showSigninModal) {
				var memberCheckoutType = settings.$dialog.find('.sm-cart-memberCheckoutType').val();

				if(memberCheckoutType != 1) {
					var submitFormAllow = false;

					settings.$dialog.submit(function(event) {
						if(!submitFormAllow) {
							...
							showSigninModal({
								isCheckout: true,
								showGuest: memberCheckoutType == 3 ? false : true,
			...
			if(typeof itemOrItemKey == 'string') {
				...
				for(var i = 0; i < _items.length; i++) {
					...
					if(itemOrItemKey.get) {
						if(
							_item.get("itemKey") == itemOrItemKey.get('itemKey') && 
							_item.get("customText") == itemOrItemKey.get('customText') && 
							_item.get("customTextLabel") == itemOrItemKey.get('customTextLabel') &&
							JSON.stringify(_item.get("fields")) == JSON.stringify(itemOrItemKey.get('fields'))
						) 
							return _item;
					} else {
						if(
							_item.get("itemKey") == itemOrItemKey.itemKey && 
							_item.get("customText") == itemOrItemKey.customText && 
							_item.get("customTextLabel") == itemOrItemKey.customTextLabel &&
							JSON.stringify(_item.get("fields")) == JSON.stringify(itemOrItemKey.fields)
						)
							return _item;
					}
				}
